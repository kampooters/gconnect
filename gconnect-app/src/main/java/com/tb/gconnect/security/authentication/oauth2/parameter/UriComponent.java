...
public class UriComponent {
	...
	public static void validate(String s, Type t, boolean template) {
		...
		if (i > -1) {
			throw new IllegalArgumentException(
					"uri.component.invalid.character");
		}
	}
	...
	/**
	 * Encodes the characters of string that are either non-ASCII characters or
	 * are ASCII characters that must be percent-encoded using the UTF-8
	 * encoding.
	 * 
	 * @param s
	 *            the string to be encoded.
	...
	private static String _encode(String s, Type t, boolean template,
	 ...
	 * component type. If a sequence of contiguous percent-encoded octets is not
	 * a valid UTF-8 character then the octets are replaced with '\uFFFD'.
	 * <p>
	...
	public static String decode(String s, Type t) {
		...
		if (s.indexOf('%') < 0) {
			...
			if (n < 2) {
				throw new IllegalArgumentException(
						"uri.component.encoded.octet.malformed");
			}
	 ...
	 * <p>
	 * Query parameter names in the returned map are always decoded. Decoding of
	 * query parameter values can be controlled using the {@code decode}
	 * parameter flag.
	 * </p>
	 * 
	 * @param u
	 *            the URI.
	 * @param decode
	 *            {@code true} if the returned query parameter values of the
	 *            query component should be in decoded form.
	 * @return the multivalued map of query parameters.
	 */
	public static MultivaluedMap<String, String> decodeQuery(URI u,
			boolean decode) {
	 ...
	 * @return the multivalued map of query parameters.
	 */
	public static MultivaluedMap<String, String> decodeQuery(String q,
			boolean decode) {
	 ...
	 * Decode the query component of a URI.
	 * <p>
	 * Decoding of query parameter names and values can be controlled using the
	 * {@code decodeNames} and {@code decodeValues} parameter flags.
	 * </p>
	 * 
	 * @param q
	 *            the query component in encoded form.
	 * @param decodeNames
	 *            {@code true} if the returned query parameter names of the
	 *            query component should be in decoded form.
	 * @param decodeValues
	 *            {@code true} if the returned query parameter values of the
	 *            query component should be in decoded form.
	 ...
	 */
	public static MultivaluedMap<String, String> decodeQuery(String q,
			boolean decodeNames, boolean decodeValues) {
		...
		do {
			...
			if (e == -1) {
				decodeQueryParam(queryParameters, q.substring(s), decodeNames,
						decodeValues);
			} else if (e > s) {
				decodeQueryParam(queryParameters, q.substring(s, e),
						decodeNames, decodeValues);
			}
			s = e + 1;
	...
	private static void decodeQueryParam(MultivaluedMap<String, String> params,
		...
		try {
			...
			if (equals > 0) {
				params.add(
						(decodeNames) ? URLDecoder.decode(
								param.substring(0, equals), "UTF-8") : param
								.substring(0, equals),
						(decodeValues) ? URLDecoder.decode(
								param.substring(equals + 1), "UTF-8") : param
								.substring(equals + 1));
			} else if (equals == 0) {
	...
	private static final class PathSegmentImpl implements PathSegment {
		...
		private static final PathSegment EMPTY_PATH_SEGMENT = new PathSegmentImpl(
				"", false);
		...
		PathSegmentImpl(String path, boolean decode,
			...
			this.path = (decode) ? UriComponent.decode(path,
					Type.PATH_SEGMENT) : path;
	 ...
	 *            then the leading '/' is ignored and is not considered a
	 *            delimiator of a path segment.
	 * @param decode
	 *            true if the path segments of the path component should be in
	 *            decoded form.
	 ...
	 * Decode the path component of a URI as path segments.
	 * <p>
	 * Any '/' character in the path is considered to be a deliminator between
	 * two path segments. Thus if the path is '/' then the path segment list
	 * will contain two empty path segments. If the path is "//" then the path
	 * segment list will contain three empty path segments. If the path is "/a/"
	 * the path segment list will consist of the following path segments in
	 * order: "", "a" and "".
	 * </p>
	 * 
	 * @param path
	 ...
	 * @param decode
	 *            {@code true} if the path segment should be in a decoded form.
	 */
	public static void decodePathSegment(List<PathSegment> segments,
			String segment, boolean decode) {
		int colon = segment.indexOf(';');
		if (colon != -1) {
			segments.add(new PathSegmentImpl((colon == 0) ? "" : segment
					.substring(0, colon), decode, decodeMatrix(segment, decode)));
		} else {
	 ...
	 */
	public static MultivaluedMap<String, String> decodeMatrix(
			String pathSegment, boolean decode) {
		...
		do {
			...
			if (e == -1) {
				...
				decodeMatrixParam(matrixMap, pathSegment.substring(s, e),
						decode);
			}
			s = e + 1;
	...
	private static void decodeMatrixParam(
			MultivaluedMap<String, String> params, String param, boolean decode) {
		int equals = param.indexOf('=');
		if (equals > 0) {
			params.add(
					UriComponent.decode(param.substring(0, equals),
							Type.MATRIX_PARAM),
					(decode) ? UriComponent.decode(param.substring(equals + 1),
							Type.MATRIX_PARAM) : param
							.substring(equals + 1));
		} else if (equals == 0) {
			// no key declared, ignore
		} else if (param.length() > 0) {
			params.add(
					UriComponent.decode(param, Type.MATRIX_PARAM),
					"");
		}
	}
	 ...
	 */
	private static ByteBuffer decodePercentEncodedOctets(String s, int i,
			ByteBuffer bb) {
		if (bb == null) {
	...
	private static int decodeHex(String s, int i) {
		...
		if (v == -1) {
			throw new IllegalArgumentException(
					"uri.component.encoded.octet.invalid.digit");
		}
		return v;
	}
 ...
 * License at http://glassfish.java.net/public/CDDL+GPL_1_1.html or
 ...
 * instance of this type is injected with PathParam, the
 * value of the annotation identifies which path segment is selected and the
 * presence of an Encoded annotation will result in an
...
interface PathSegment {
	...
	/**
	 * Get a map of the matrix parameters associated with the path segment. The
	 * map keys are the names of the matrix parameters with any percent-escaped
	 * octets decoded.
	 * 
